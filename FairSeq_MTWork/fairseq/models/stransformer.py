...
class STransformerModel(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        encoder = STransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = STransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return STransformerModel(encoder, decoder)


class STransformerEncoder(FairseqEncoder):
    """
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if self.normalize:
            for i in range(len(encoder_out_list)):
                encoder_out_list[i] = self.layer_norm(encoder_out_list[i])
...
class STransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, left_pad=False, final_norm=True):
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            STransformerDecoderLayer(args, no_encoder_attn)
...
class STransformerDecoderLayer(nn.Module):
    """Decoder layer block.
...
@register_model_architecture('stransformer', 'stransformer')
...
@register_model_architecture('stransformer', 'stransformer_iwslt_de_en')
...
@register_model_architecture('stransformer', 'stransformer_t2t_iwslt_de_en')
