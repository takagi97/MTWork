...
@register_model('dense_all_transformer')
class DenseAllTransformerModel(FairseqModel):
    """
    ...
    def build_model(cls, args, task):
        ...
        encoder = DenseAllTransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = DenseAllTransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return DenseAllTransformerModel(encoder, decoder)


class DenseAllTransformerEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        self.layers.extend([
            DenseAllTransformerEncoderLayer(args)
        ...
        # change
        self.weight = torch.Tensor([i for i in range(args.encoder_layers)])
        self.w_softmax = torch.nn.Softmax(dim=0)
        self.weight = self.w_softmax(self.weight)
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        encoder_out = self.weight[0] * encoder_out_list[0]
        for i,temp in enumerate(encoder_out_list):
            if i > 0:
                encoder_out = encoder_out + self.weight[i] * temp
...
class DenseAllTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, left_pad=False, final_norm=True):
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            DenseAllTransformerDecoderLayer(args, no_encoder_attn)
...
class DenseAllTransformerEncoderLayer(nn.Module):
    """Encoder layer block.
...
class DenseAllTransformerDecoderLayer(nn.Module):
    """Decoder layer block.
...
@register_model_architecture('dense_all_transformer', 'dense_all_transformer')
...
@register_model_architecture('dense_all_transformer', 'dense_all_transformer_iwslt_de_en')
...
@register_model_architecture('dense_all_transformer', 'dense_all_transformer_t2t_iwslt_de_en')
