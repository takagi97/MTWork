...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        self.grus = nn.ModuleList(nn.GRUCell(embed_dim, embed_dim) for _ in range(args.encoder_layers))
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
        len_size, batch_size, hidden_size = x.size()
        ...
        for index, layer in enumerate(self.layers):
            for j in range(self.calculate_num):
                ...
                x = self.grus[index](x.view(-1, hidden_size)).view(len_size, batch_size, hidden_size)
